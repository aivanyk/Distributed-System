...
func (cfg *config) apply(server int, m ApplyMsg) {
	...
	fmt.Printf("Server: %d nextInt is: %d \n", server, cfg.nextIndex[server])
...
func (cfg *config) nCommitted(index int) (int, logEntry) {
	...
	for i := 0; i < len(cfg.paxos); i++ {
		...
		// log.Warn().Msgf("nCommitted for server %v - ok %v, nextIndex %v, index %v", i, cfg.logs[i], nextIndex, index)
		cfg.mu.Unlock()
...
func (cfg *config) wait(index int, n int, startballot int) (bool, logEntry) {
	...
	log.Warn().Msgf("nCommitted for index %d is nd %d log %v", index, nd, e)
	if nd < n {
