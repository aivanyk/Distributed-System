...
func TestBackup2B(t *testing.T) {
	...
	log.Info().Msgf("Tester leader2 %d other %d", leader2, other)
	if leader2 == other {
	...
	cfg.connect(other)
	backLeader := cfg.checkOneLeader()
	log.Info().Msgf("Bring original leader back leader %d", backLeader)
	...
	log.Info().Msgf("final leader %d", finalLeader)
	cfg.one(rand.Int(), servers, true)
...
loop:
	for try := 0; try < 5; try++ {
		...
		log.Info().Msgf("log before getstate")
		failed := false
...
func TestFig5CLogReplication2B(t *testing.T) {
	...
	log.Warn().Msgf("Final leader is %v", leader3)
...
func TestPersist12C(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	log.Warn().Msgf("Disconnecting leader1 %v\n", leader1)
	cfg.disconnect(leader1)
	...
	leader2 := cfg.checkOneLeader()
	log.Warn().Msgf("Disconnecting leader2 %v\n", leader2)
	cfg.disconnect(leader2)
	index := cfg.one(14, servers-1, true)
	cfg.start1(leader2, cfg.applier)
	cfg.connect(leader2)
	log.Warn().Msgf("Connecting leader2 %v\n", leader2)
